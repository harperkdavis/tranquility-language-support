...
export function activate(context: vscode.ExtensionContext) {
	...
	const builtinFunctions: TranqFunc[] = [
		{
			name: 'alloc',
			args: ['n'],
	...
	const examine = (document: vscode.TextDocument): TranqExam => {
		const lines = document.getText().split('\n');
		const functions = [...builtinFunctions];
		const globals = [];
		let currentFunction: TranqFunc | null = null;
		let braceDepth = 0;
	...
	const availableVariables = (exam: TranqExam, position: vscode.Position): TranqVar[] => {
	...
	const formatFunction = (fn: TranqFunc) => {
		return `${fn.name}(${fn.args.map(a => `${a}`).join(', ')})`;
	};

	const functionDocs = (fn: TranqFunc): string => {
